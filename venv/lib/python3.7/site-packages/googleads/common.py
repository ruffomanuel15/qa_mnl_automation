...
class ProxyConfig(object):
  ...
  def BuildOpener(self):
    ...
    """
    return urllib.request.build_opener(*self.GetHandlers())
...
class GoogleSchemaHelper(object, metaclass=abc.ABCMeta):
...
class GoogleSoapService(object, metaclass=abc.ABCMeta):
...
class ZeepServiceProxy(GoogleSoapService):
  ...
  def _PackArgumentsHelper(self, elem, data, set_type_attrs):
    ...
    if isinstance(data, dict):  # Instantiate from simple Python dict
      ...
      data_formatted = iter(data.items())
      ...
      data_formatted = list(zip(dir(data), [data[k] for k in dir(data)]))
      packed_result = self._CreateComplexTypeFromData(
  ...
  def _DiscoverElementTypeFromLocalname(self, type_localname):
    ...
    elem_type = None
    last_exception = None
    for ns_prefix in list(self.zeep_client.wsdl.types.prefix_map.values()):
      try:
...
class HeaderHandler(object, metaclass=abc.ABCMeta):
